...
- 실제 발생 가능성이 없거나 극히 낮은 상황을 미리 가정해 불필요한 fallback·분기·방어 로직을 추가하지 않는다. 예외 처리는 확인된 요구사항이나 현실적인 실패 경로에 근거한다.
- 대상을 지칭할 때는 현재 상황에서 무엇을 가리키는지 구체적으로 밝힌다. `프로세스가 죽는다`처럼 범위가 모호한 표현 대신 `MCP 서버 프로세스가 종료된다`, `Website 개발 서버가 종료된다`처럼 구성요소와 역할을 명시한다.
- 코드를 작성할 때 변수 이름은 역할·값·단위를 문맥 없이도 직관적으로 이해할 수 있게 짓는다. 의미가 불분명한 축약이나 임시 이름을 피한다.
- 새로 작성하거나 실질적으로 수정하는 코드에는 논리적 블록 단위로 의도와 동작을 설명하는 간결한 주석을 추가해 사용자가 코드를 쉽게 읽고 이해할 수 있게 한다. 코드만으로 자명한 내용을 되풀이하거나 줄마다 주석을 달지는 않는다.
