...
link to the Markdown file, labeled with its repository-relative path. Before reporting
success, resolve each exported local link target from the Markdown file's directory
after removing any `#L...` fragment and confirm that the target exists. If Markdown,
SVG, or link validation fails, report the failure instead of claiming that the file
was created.
