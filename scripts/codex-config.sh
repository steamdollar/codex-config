...
rollback_install() {
  local status=${1:-$?}
  trap - ERR INT TERM
  if ((status != 0)) && { [[ -n "$active_backup" ]] || [[ "$retired_review_removed" == true ]] || [[ "$retired_sub_agents_removed" == true ]] || [[ "$retired_planner_removed" == true ]] || [[ "$retired_advisor_removed" == true ]] || [[ "$retired_terra_executor_removed" == true ]] || [[ -n "${retired_agent_removed[*]-}" ]]; }; then
...
install_apply() {
  ...
  if [[ -n "${retired_agent_cleanup_required[*]-}" ]]; then
    changes_required=true
  fi
